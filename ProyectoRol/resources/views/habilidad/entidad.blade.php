<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-primary leading-tight">
            {{ __('Añadir Habilidades a: ') }} {{$entidad->nombre}}
        </h2> 
    </x-slot>
    @push('head')
	 <!-- Styles -->
	...
	<form action="{{ route('entidad.addHabilidad', $entidad->id) }}" method="POST">
            @csrf 
            @method('PATCH')
                    ...
                    <table id="pjs" class="hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                        <thead>
                            <tr role="row">
                                ...
                                <th data-priority="2">Negativo</th>
                                <th data-priority="2">Escalado</th>
                                <th data-priority="3">Nivel</th>
                                <th data-priority="3">Dado</th>
                                <th data-priority="3">Valor</th>
                                <th data-priority="3">Tipo</th>
                                <th data-priority="3">Focus</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($habilidades as $habilidad)
                                <tr role="row">
                                    <td tabindex="0" class="sorting_1" style><input type="checkbox" value="{{$habilidad->id}}" name="{{$habilidad->id}}" id="{{$entidad->id}}">
                                    <td class="text-center">{{$habilidad->nombre}}</td>
                                    @if($habilidad->facultadNegativa != "null") 
                                        <td class="text-center">{{$habilidad->facultadNegativa}} --> {{$habilidad->baseFacultadNegativa}}</td>
                                    @else
                                        <td class="text-center">No</td>
                                    @endif
                                    <td class="text-center">{{$habilidad->facultadPositiva}}</td>
                                    <td class="text-center">{{$habilidad->nivelHabilidad}}</td>
                                    <td class="text-center">{{$habilidad->dado}}</td>
                                    <td class="text-center">{{$habilidad->valorBase}}</td>
                                    @if($habilidad->ataque) 
                                        <td class="text-center">Ataque</td>
                                    @else
                                        <td class="text-center">Curación</td>
                                    @endif
                                    @if($habilidad->area) 
